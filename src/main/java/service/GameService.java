...
public class GameService {
    ...
    public boolean isMafiaCaught() {
        ...
        long mafiaCount = currentGame.getParticipants().stream()
                .filter(person -> person.getPersonType() == PersonType.MAFIA && !person.isDead())
                .count();
        return mafiaCount == 0;
    }
    ...
    public void investigatePerson(Person person) {
        ...
        System.out.println("머리 색깔: " + person.getHairType());
        System.out.println("피부 색깔: " + person.getColor());
        // 추가적인 특성 정보 출력
    }
    ...
    public boolean accusePerson(Person person) {
        ...
        if (person.getPersonType() == PersonType.MAFIA) {
